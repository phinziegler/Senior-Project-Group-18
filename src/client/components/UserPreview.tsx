    ...
    render() {
        return (<>
            <div
                ...
                {/* USERNAME TEXT */}
                <span title={this.props.user?.username == this.props.username ? 'You' : ''} className={'h5 me-1 text-break ' + (this.props.user?.username == this.props.username ? 'text-success' : '')} style={{ lineHeight: '30px' }}>{this.props.username}</span>
                
                {/* LOBBY OWNER INDICATOR */}
                {this.props.owner && <span className='ms-2' title="Lobby Owner">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="lightgreen" className="bi bi-star icon" viewBox="0 0 16 16">
                    ...
                    <span>
                        {!this.state.isFriend
                            ...
                            <>
                                ...
                                {/* ADD FRIEND BUTTON */}
                                {this.props.user?.username != this.props.username && <button title="Add Friend" style={{ backgroundColor: 'transparent', border: 'none' }} onClick={() => this.addFriend()}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-person-fill-add glow-button-friend icon" viewBox="0 0 16 16" style={{ margin: '0px' }}>
                            ...
                            <span className="text-success" title="Friend Added">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-person-fill-check icon me-1" viewBox="0 0 16 16">
                        ...
                        {this.props.admin && this.props.user?.username != this.props.username && <button title="Kick From Lobby" onClick={() => this.props.removeUser(this.props.username)} className="text-danger btn-svg" style={{ backgroundColor: 'transparent' }}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-person glow-button-kick icon" viewBox="0 -1 16 16">
