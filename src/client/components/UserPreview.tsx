    ...
    render() {
        return (<>
            <div
                ...
                <span title={this.props.user?.username == this.props.username ? 'You' : ''} className={'h2 ' + (this.props.user?.username == this.props.username ? 'text-success' : '')}>{this.props.username}</span>
                ...
                {this.state.showExtra &&
                    <span className="ms-1">
                        {!this.state.isFriend
                            ...
                            {this.props.admin && this.props.user?.username != this.props.username && <button title="Kick From Lobby" onClick={() => this.props.removeUser(this.props.username)} className="text-danger" style={{backgroundColor: 'transparent'}}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" className="bi bi-person-fill-x glow-button-kick icon" viewBox="0 -1 16 16">
